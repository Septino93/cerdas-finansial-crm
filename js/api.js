...
const api={
 ...
 async uploadClientDocument(clientId,file,meta={}){const safe=String(file.name||'file').replace(/[^a-zA-Z0-9._-]/g,'-'),path=`${clientId}/${Date.now()}-${safe}`;const {error}=await db.storage.from('client-documents').upload(path,file,{upsert:false,contentType:file.type||undefined});if(error)throw error;return mustData(db.from('activity_logs').insert({client_id:clientId,consultation_id:meta.consultationId||null,event_type:'document_uploaded',description:`Dokumen diupload: ${meta.title||file.name}`,metadata:{title:meta.title||file.name,category:meta.category||'Lainnya',filename:file.name,path,size:file.size,type:file.type,public_for_client:Boolean(meta.publicForClient)}}).select().single())},
