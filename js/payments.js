...
async function loadPaymentClients(){
 ...
 manualPaymentClient.innerHTML='<option value="">Pilih client</option>'+paymentClients.filter(c=>c.is_active!==false).map(c=>`<option value="${esc(c.id)}">${esc(c.full_name)} · ${esc(c.whatsapp||c.email||'-')}</option>`).join('');
}
function openPaymentModal(){paymentAddModal.hidden=false;paymentAddModal.setAttribute('aria-hidden','false');document.body.classList.add('client-modal-open');requestAnimationFrame(()=>paymentAddModal.classList.add('is-open'));setTimeout(()=>manualPaymentClient.focus(),180)}
function closePaymentModal(){paymentAddModal.classList.remove('is-open');document.body.classList.remove('client-modal-open');setTimeout(()=>{paymentAddModal.hidden=true;paymentAddModal.setAttribute('aria-hidden','true')},240)}
function bindPaymentModal(){
 openPaymentModal.addEventListener('click',openPaymentModal);
 ...
 manualPaymentForm.addEventListener('submit',createManualPayment);
}
async function createManualPayment(event){
 event.preventDefault();
 const button=saveManualPaymentBtn;
 button.disabled=true;button.textContent='Membuat...';
 try{
  const result=await api.createManualPayment({clientId:manualPaymentClient.value,title:manualPaymentName.value,amount:manualPaymentAmount.value,notes:manualPaymentNotes.value});
  manualPaymentForm.reset();closePaymentModal();await renderPayments();
...
async function renderPayments(){
 ...
 }catch(err){paymentList.innerHTML=`<div class="empty"><strong>Gagal memuat</strong>${esc(err.message)}</div>`}
}
